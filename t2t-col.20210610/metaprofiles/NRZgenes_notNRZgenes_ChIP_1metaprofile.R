...
ChIP_notNRZgeneMats <- mclapply(seq_along(ChIPNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(ChIPDirs[x], "NRZgeneProfiles/matrices/",
                                ChIPNames[x],
                                "_MappedOn_", refbase[x], "_lowXM_", align, "_sort_norm_notNRZgenes_in_",
                                chrName[y], "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = F, skip = 3))
  })
}, mc.cores = length(ChIPNames))
...
control_NRZgeneMats <- mclapply(seq_along(controlNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(controlDirs[x], "NRZgeneProfiles/matrices/",
                                controlNames[x],
                                "_MappedOn_", refbase[x], "_lowXM_", align, "_sort_norm_NRZgenes_in_",
                                chrName[y], "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = F, skip = 3))
...
## control
# notNRZgene
control_notNRZgeneMats <- mclapply(seq_along(controlNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(controlDirs[x], "NRZgeneProfiles/matrices/",
                                controlNames[x],
                                "_MappedOn_", refbase[x], "_lowXM_", align, "_sort_norm_notNRZgenes_in_",
...
# If notNRZgenes from multiple chromosomes are to be analysed,
# concatenate the corresponding notNRZgene coverage matrices
control_notNRZgeneMats <- mclapply(seq_along(control_notNRZgeneMats), function(x) {
  if(length(chrName) > 1) {
    do.call(rbind, control_notNRZgeneMats[[x]])
  } else {
    control_notNRZgeneMats[[x]][[1]]
  }
}, mc.cores = length(control_notNRZgeneMats))

## control
# notNRZranLoc
control_notNRZranLocMats <- mclapply(seq_along(controlNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(controlDirs[x], "NRZgeneProfiles/matrices/",
                                controlNames[x],
                                "_MappedOn_", refbase[x], "_lowXM_", align, "_sort_norm_notNRZgenes_in_",
...
# If notNRZranLocs from multiple chromosomes are to be analysed,
# concatenate the corresponding notNRZranLoc coverage matrices
control_notNRZranLocMats <- mclapply(seq_along(control_notNRZranLocMats), function(x) {
  if(length(chrName) > 1) {
    do.call(rbind, control_notNRZranLocMats[[x]])
  } else {
    control_notNRZranLocMats[[x]][[1]]
  }
}, mc.cores = length(control_notNRZranLocMats))
...
# notNRZgene
log2ChIP_notNRZgeneMats <- mclapply(seq_along(ChIP_notNRZgeneMats), function(x) {
  print(paste0(ChIPNames[x], " library; using ", controlNames[x], " for log2((ChIP+1)/(input+1)) calculation"))
  log2((ChIP_notNRZgeneMats[[x]]+1)/(control_notNRZgeneMats[[x]]+1))
}, mc.cores = length(ChIP_notNRZgeneMats))

# notNRZranLoc
log2ChIP_notNRZranLocMats <- mclapply(seq_along(ChIP_notNRZranLocMats), function(x) {
  print(paste0(ChIPNames[x], " library; using ", controlNames[x], " for log2((ChIP+1)/(input+1)) calculation"))
  log2((ChIP_notNRZranLocMats[[x]]+1)/(control_notNRZranLocMats[[x]]+1))
}, mc.cores = length(ChIP_notNRZranLocMats))


# log2ChIP
# Add column names
for(x in seq_along(log2ChIP_NRZgeneMats)) {
  ...
  colnames(log2ChIP_notNRZgeneMats[[x]]) <- c(paste0("u", 1:(upstream/binSize)),
                                              paste0("t", ((upstream/binSize)+1):((upstream+bodyLength)/binSize)),
                                              paste0("d", (((upstream+bodyLength)/binSize)+1):(((upstream+bodyLength)/binSize)+(downstream/binSize))))

  colnames(log2ChIP_notNRZranLocMats[[x]]) <- c(paste0("u", 1:(upstream/binSize)),
                                                paste0("t", ((upstream/binSize)+1):((upstream+bodyLength)/binSize)),
                                                paste0("d", (((upstream+bodyLength)/binSize)+1):(((upstream+bodyLength)/binSize)+(downstream/binSize))))
}
...
log2ChIP_mats <- mclapply(seq_along(log2ChIP_NRZgeneMats), function(x) {
  list(
       # NRZgenes
       ...
       # notNRZgenes
       log2ChIP_notNRZgeneMats[[x]],
       # notNRZranLocs
       log2ChIP_notNRZranLocMats[[x]]
      )
}, mc.cores = length(log2ChIP_NRZgeneMats))
...
notNRZgenesTmp <- lapply(seq_along(summaryDFfeature_list_log2ChIP), function(x) {
  summaryDFfeature_list_log2ChIP[[x]][[3]]
})
notNRZranLocTmp <- lapply(seq_along(summaryDFfeature_list_log2ChIP), function(x) {
  summaryDFfeature_list_log2ChIP[[x]][[4]]
})
names(NRZgenesTmp) <- log2ChIPNamesPlot
names(NRZranLocTmp) <- log2ChIPNamesPlot
names(notNRZgenesTmp) <- log2ChIPNamesPlot
names(notNRZranLocTmp) <- log2ChIPNamesPlot
...
# control
# Add column names
for(x in seq_along(control_NRZgeneMats)) {
  ...
  colnames(control_notNRZgeneMats[[x]]) <- c(paste0("u", 1:(upstream/binSize)),
                                              paste0("t", ((upstream/binSize)+1):((upstream+bodyLength)/binSize)),
                                              paste0("d", (((upstream+bodyLength)/binSize)+1):(((upstream+bodyLength)/binSize)+(downstream/binSize))))

  colnames(control_notNRZranLocMats[[x]]) <- c(paste0("u", 1:(upstream/binSize)),
                                                paste0("t", ((upstream/binSize)+1):((upstream+bodyLength)/binSize)),
                                                paste0("d", (((upstream+bodyLength)/binSize)+1):(((upstream+bodyLength)/binSize)+(downstream/binSize))))
}
...
# Create list of lists in which each element in the enclosing list corresponds to a library
# and the two elements in the nested list correspond to coverage matrices for features and random loci
control_mats <- mclapply(seq_along(control_NRZgeneMats), function(x) {
  list(
       # NRZgenes
       ...
       # notNRZgenes
       control_notNRZgeneMats[[x]],
       # notNRZranLocs
       control_notNRZranLocMats[[x]]
      )
}, mc.cores = length(control_NRZgeneMats))
...
notNRZgenesTmp <- lapply(seq_along(summaryDFfeature_list_control), function(x) {
  summaryDFfeature_list_control[[x]][[3]]
})
notNRZranLocTmp <- lapply(seq_along(summaryDFfeature_list_control), function(x) {
  summaryDFfeature_list_control[[x]][[4]]
})
names(NRZgenesTmp) <- controlNamesPlot
names(NRZranLocTmp) <- controlNamesPlot
names(notNRZgenesTmp) <- controlNamesPlot
names(notNRZranLocTmp) <- controlNamesPlot
