...
# ./LRZgenes_NRZgenes_armgenes_RNA_1metaprofile.R 'Chr1,Chr2,Chr3,Chr4,Chr5' both 2000 2000 '2kb' 10 10bp '0.02,0.96' 'Col_0_RNAseq_Rep1_ERR966157' 'RNAseq_leaf_Rigal_Mathieu_2016_PNAS/snakemake_RNAseq_STAR_t2t-col.20210610' 'RNA-seq (leaf)' 'navy' 't2t-col.20210610'
# conda deactivate
...
ChIP_LRZgeneMats <- mclapply(seq_along(ChIPNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(ChIPDirs[x], "LRZ_NRZgeneProfiles/matrices/",
                                ChIPNames[x],
                                "_MappedOn_", refbase[x], "_", align, "_sort_norm_LRZgenes_in_",
...
ChIP_armgeneMats <- mclapply(seq_along(ChIPNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(ChIPDirs[x], "LRZ_NRZgeneProfiles/matrices/",
                                ChIPNames[x],
                                "_MappedOn_", refbase[x], "_", align, "_sort_norm_armgenes_in_",
                                chrName[y], "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = F, skip = 3))
  })
}, mc.cores = length(ChIPNames))
