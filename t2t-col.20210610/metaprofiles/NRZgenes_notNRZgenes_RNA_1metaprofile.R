...
# ./NRZgenes_notNRZgenes_RNA_1metaprofile.R 'Chr1,Chr2,Chr3,Chr4,Chr5' both 2000 2000 '2kb' 10 10bp '0.02,0.96' 'Col_0_RNAseq_Rep1_ERR966157' 'RNAseq_leaf_Rigal_Mathieu_2016_PNAS/snakemake_RNAseq_STAR_t2t-col.20210610' 'RNA-seq (leaf)' 'navy' 't2t-col.20210610'
# conda deactivate
...
#                                        split = ",")))
#ChIPNames <- unlist(strsplit("Col_0_RNAseq_Rep1_ERR966157",
#                             split = ","))
#ChIPNamesDir <- unlist(strsplit("RNAseq_leaf_Rigal_Mathieu_2016_PNAS/snakemake_RNAseq_STAR_t2t-col.20210610",
#                                split = ","))
#ChIPNamesPlot <- unlist(strsplit("RNA-seq (leaf)",
#                                 split = ","))
#ChIPColours <- unlist(strsplit("navy",
#                               split = ","))
...
ChIPColours <- unlist(strsplit(args[12],
                               split = ","))
refbase <- unlist(strsplit(args[13],
                           split = ","))
...
ChIP_NRZgeneMats <- mclapply(seq_along(ChIPNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(ChIPDirs[x], "NRZgeneProfiles/matrices/",
                                ChIPNames[x],
                                "_MappedOn_", refbase[x], "_", align, "_sort_norm_NRZgenes_in_",
...
ChIP_notNRZgeneMats <- mclapply(seq_along(ChIPNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(ChIPDirs[x], "NRZgeneProfiles/matrices/",
                                ChIPNames[x],
                                "_MappedOn_", refbase[x], "_", align, "_sort_norm_notNRZgenes_in_",
                                chrName[y], "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = F, skip = 3))
  })
}, mc.cores = length(ChIPNames))
