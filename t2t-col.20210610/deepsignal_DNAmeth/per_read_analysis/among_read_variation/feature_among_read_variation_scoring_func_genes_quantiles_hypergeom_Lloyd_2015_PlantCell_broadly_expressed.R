...
# ./feature_among_read_variation_scoring_func_genes_quantiles_hypergeom_Lloyd_2015_PlantCell_broadly_expressed.R Col_0_deepsignalDNAmeth_30kb_90pc t2t-col.20210610 CpG 0.50 'Chr1,Chr2,Chr3,Chr4,Chr5' 'gene' 'regions'
# conda deactivate
...
ggsave(paste0(plotDir_kappa_mC,
              ...
              "_broadly_expressed_genes_hypergeomTest_quantiles.pdf"),
...
# Run hypergeometric test on each group of genes to evaluate
# representation of broadly_expressed genes
hgTest_alpha_broadly_expressed_list <- lapply(seq_along(filt_alpha_mC_groups_featID), function(x) {
  hgTest(group = x,
...
hgTest_alpha_broadly_expressed_DF <- dplyr::bind_rows(hgTest_alpha_broadly_expressed_list)
hgTest_alpha_broadly_expressed_DF$group <- as.character(hgTest_alpha_broadly_expressed_DF$group)

hgTest_alpha_broadly_expressed_DF$BHadj_pval <- p.adjust(hgTest_alpha_broadly_expressed_DF$pval, method = "BH")

gg_hgTest_alpha_broadly_expressed <- ggplot(data = hgTest_alpha_broadly_expressed_DF,
                                 mapping = aes(x = group,
                                               y = log2obsexp,
                                               colour = pval,
                                               size = observed)) +
...
#  labs(size = "Count", colour = bquote("BH-adjusted" ~ italic(P))) +
  ...
  ylab(bquote(atop("Log"[2] * "(observed/expected) broadly expressed genes"))) +
  ylim(-max(abs(hgTest_alpha_broadly_expressed_DF$log2obsexp)), max(abs(hgTest_alpha_broadly_expressed_DF$log2obsexp))) +
  theme_bw()
ggsave(paste0(plotDir_alpha_mC,
              ...
              "_", paste0(chrName, collapse = "_"), "_", context,
              "_broadly_expressed_genes_hypergeomTest_quantiles.pdf"),
       plot = gg_hgTest_alpha_broadly_expressed,
       height = 5, width = 6)
write.table(hgTest_alpha_broadly_expressed_DF,
            paste0(outDir,
                   sampleName, "_filt_df_ka_alpha_all_mean_mC_all_",
                   featName, "_", featRegion,
                   "_", paste0(chrName, collapse = "_"), "_", context,
