...
# ./feature_among_read_variation_scoring_func_genes_quantiles_hypergeom_Lloyd_2015_PlantCell_gbM.R Col_0_deepsignalDNAmeth_30kb_90pc t2t-col.20210610 CpG 0.50 'Chr1,Chr2,Chr3,Chr4,Chr5' 'gene' 'regions'
# conda deactivate
...
gbM <- ds3[which(ds3$Gene.body.methylated == 1),]
nrow(gbM)
...
hgTest_kappa_gbM_list <- lapply(seq_along(filt_kappa_mC_groups_featID), function(x) {
  hgTest(group = x,
...
hgTest_kappa_gbM_DF <- dplyr::bind_rows(hgTest_kappa_gbM_list)
hgTest_kappa_gbM_DF$group <- as.character(hgTest_kappa_gbM_DF$group)

hgTest_kappa_gbM_DF$BHadj_pval <- p.adjust(hgTest_kappa_gbM_DF$pval, method = "BH")

gg_hgTest_kappa_gbM <- ggplot(data = hgTest_kappa_gbM_DF,
                                 mapping = aes(x = group,
                                               y = log2obsexp,
                                               colour = pval,
                                               size = observed)) +
...
#  labs(size = "Count", colour = bquote("BH-adjusted" ~ italic(P))) +
  ...
  ylim(-max(abs(hgTest_kappa_gbM_DF$log2obsexp)), max(abs(hgTest_kappa_gbM_DF$log2obsexp))) +
  theme_bw()
ggsave(paste0(plotDir_kappa_mC,
              ...
              "_gbM_genes_hypergeomTest_quantiles.pdf"),
       plot = gg_hgTest_kappa_gbM,
       height = 5, width = 6)
...
# Run hypergeometric test on each group of genes to evaluate
# representation of gbM genes
hgTest_stocha_gbM_list <- lapply(seq_along(filt_stocha_mC_groups_featID), function(x) {
  hgTest(group = x,
...
hgTest_stocha_gbM_DF <- dplyr::bind_rows(hgTest_stocha_gbM_list)
hgTest_stocha_gbM_DF$group <- as.character(hgTest_stocha_gbM_DF$group)

hgTest_stocha_gbM_DF$BHadj_pval <- p.adjust(hgTest_stocha_gbM_DF$pval, method = "BH")

gg_hgTest_stocha_gbM <- ggplot(data = hgTest_stocha_gbM_DF,
                                mapping = aes(x = group,
                                              y = log2obsexp,
                                              colour = pval,
                                              size = observed)) +
...
#  labs(size = "Count", colour = bquote("BH-adjusted" ~ italic(P))) +
  ...
  ylab(bquote(atop("Log"[2] * "(observed/expected) gbM genes"))) +
  ylim(-max(abs(hgTest_stocha_gbM_DF$log2obsexp)), max(abs(hgTest_stocha_gbM_DF$log2obsexp))) +
  theme_bw()
ggsave(paste0(plotDir_stocha_mC,
              ...
              "_", paste0(chrName, collapse = "_"), "_", context,
              "_gbM_genes_hypergeomTest_quantiles.pdf"),
       plot = gg_hgTest_stocha_gbM,
       height = 5, width = 6)
write.table(hgTest_stocha_gbM_DF,
            paste0(outDir,
                   sampleName, "_filt_df_mean_stocha_all_mean_mC_all_",
                   featName, "_", featRegion,
                   "_", paste0(chrName, collapse = "_"), "_", context,
