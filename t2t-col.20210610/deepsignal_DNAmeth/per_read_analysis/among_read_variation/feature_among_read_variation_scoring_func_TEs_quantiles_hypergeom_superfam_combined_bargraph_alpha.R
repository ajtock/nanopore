...
# ./feature_among_read_variation_scoring_func_TEs_quantiles_hypergeom_superfam_combined_bargraph_alpha.R Col_0_deepsignalDNAmeth_30kb_90pc t2t-col.20210610 CHG 0.50 'Chr1,Chr2,Chr3,Chr4,Chr5' 'TE' 'bodies' 10000 1000
# conda deactivate
...
superfamList <- lapply(1:length(superfamNames), function(y) {
  superfam <-  read.table(paste0(outDir,
                          ...
                          header = T)
  superfam <- data.frame(Feature = rep(superfamNamesPlot[y], 9),
                         superfam)
  superfam
})
