...
# ./feature_among_read_variation_scoring_func_genes_quantiles_hypergeom_Lloyd_2015_PlantCell_lethal.R Col_0_deepsignalDNAmeth_30kb_90pc t2t-col.20210610 CpG 0.50 'Chr1,Chr2,Chr3,Chr4,Chr5' 'gene' 'regions'
# conda deactivate
...
ggsave(paste0(plotDir_kappa_mC,
              ...
              "_lethal_genes_hypergeomTest_quantiles.pdf"),
...
ggsave(paste0(plotDir_kappa_mC,
              ...
              featName, "_", featRegion,
              "_", paste0(chrName, collapse = "_"), "_", context,
              "_nonlethal_genes_hypergeomTest_quantiles.pdf"),
...
# Run hypergeometric test on each group of genes to evaluate
# representation of lethal genes
hgTest_alpha_lethal_list <- lapply(seq_along(filt_alpha_mC_groups_featID), function(x) {
  hgTest(group = x,
...
hgTest_alpha_lethal_DF <- dplyr::bind_rows(hgTest_alpha_lethal_list)
hgTest_alpha_lethal_DF$group <- as.character(hgTest_alpha_lethal_DF$group)

hgTest_alpha_lethal_DF$BHadj_pval <- p.adjust(hgTest_alpha_lethal_DF$pval, method = "BH")

gg_hgTest_alpha_lethal <- ggplot(data = hgTest_alpha_lethal_DF,
                                 mapping = aes(x = group,
                                               y = log2obsexp,
                                               colour = pval,
                                               size = observed)) +
...
#  labs(size = "Count", colour = bquote("BH-adjusted" ~ italic(P))) +
  ...
  ylab(bquote(atop("Log"[2] * "(observed/expected) lethal genes"))) +
  ylim(-max(abs(hgTest_alpha_lethal_DF$log2obsexp)), max(abs(hgTest_alpha_lethal_DF$log2obsexp))) +
  theme_bw()
ggsave(paste0(plotDir_alpha_mC,
              ...
              "_", paste0(chrName, collapse = "_"), "_", context,
              "_lethal_genes_hypergeomTest_quantiles.pdf"),
       plot = gg_hgTest_alpha_lethal,
       height = 5, width = 6)
write.table(hgTest_alpha_lethal_DF,
            paste0(outDir,
                   sampleName, "_filt_df_ka_alpha_all_mean_mC_all_",
                   featName, "_", featRegion,
                   "_", paste0(chrName, collapse = "_"), "_", context,
...
# Run hypergeometric test on each group of genes to evaluate
# representation of nonlethal genes
hgTest_alpha_nonlethal_list <- lapply(seq_along(filt_alpha_mC_groups_featID), function(x) {
  hgTest(group = x,
         group_feat_list = filt_alpha_mC_groups_featID,
         genome_feat = filt_alpha_mC_groups_featID_universe,
         genome_feat_query = featID_nonlethal,
...
hgTest_alpha_nonlethal_DF <- dplyr::bind_rows(hgTest_alpha_nonlethal_list)
hgTest_alpha_nonlethal_DF$group <- as.character(hgTest_alpha_nonlethal_DF$group)

hgTest_alpha_nonlethal_DF$BHadj_pval <- p.adjust(hgTest_alpha_nonlethal_DF$pval, method = "BH")

gg_hgTest_alpha_nonlethal <- ggplot(data = hgTest_alpha_nonlethal_DF,
                                    mapping = aes(x = group,
                                                  y = log2obsexp,
                                                  colour = pval,
                                                  size = observed)) +
...
#  labs(size = "Count", colour = bquote("BH-adjusted" ~ italic(P))) +
  ...
  xlab(bquote(atop("Krippendorff's alpha and mean m" * .(context), .(featName) ~ .(featRegion) ~ "group"))) +
  ylab(bquote(atop("Log"[2] * "(observed/expected) nonlethal genes"))) +
  ylim(-max(abs(hgTest_alpha_nonlethal_DF$log2obsexp)), max(abs(hgTest_alpha_nonlethal_DF$log2obsexp))) +
  theme_bw()
ggsave(paste0(plotDir_alpha_mC,
              sampleName, "_filt_df_ka_alpha_all_mean_mC_all_",
              featName, "_", featRegion,
              "_", paste0(chrName, collapse = "_"), "_", context,
              "_nonlethal_genes_hypergeomTest_quantiles.pdf"),
       plot = gg_hgTest_alpha_nonlethal,
       height = 5, width = 6)
write.table(hgTest_alpha_nonlethal_DF,
            paste0(outDir,
                   sampleName, "_filt_df_ka_alpha_all_mean_mC_all_",
                   featName, "_", featRegion,
                   "_", paste0(chrName, collapse = "_"), "_", context,
