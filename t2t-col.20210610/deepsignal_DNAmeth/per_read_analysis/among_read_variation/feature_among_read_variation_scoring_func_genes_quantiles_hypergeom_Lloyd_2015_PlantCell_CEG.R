...
# ./feature_among_read_variation_scoring_func_genes_quantiles_hypergeom_Lloyd_2015_PlantCell_CEG.R Col_0_deepsignalDNAmeth_30kb_90pc t2t-col.20210610 CpG 0.50 'Chr1,Chr2,Chr3,Chr4,Chr5' 'gene' 'regions'
# conda deactivate
...
CEG <- ds3[which(ds3$Core.eukaryotic.gene == 1),]
nrow(CEG)
...
featID_CEG <- unique(featDF[which(featDF$parent %in% CEG[,1]),]$parent)
...
# representation of CEG genes
hgTest_kappa_CEG_list <- lapply(seq_along(filt_kappa_mC_groups_featID), function(x) {
  hgTest(group = x,
         group_feat_list = filt_kappa_mC_groups_featID,
         genome_feat = filt_kappa_mC_groups_featID_universe,
         genome_feat_query = featID_CEG,
         samples_num = 1e5)
})

hgTest_kappa_CEG_DF <- dplyr::bind_rows(hgTest_kappa_CEG_list)
hgTest_kappa_CEG_DF$group <- as.character(hgTest_kappa_CEG_DF$group)

hgTest_kappa_CEG_DF$BHadj_pval <- p.adjust(hgTest_kappa_CEG_DF$pval, method = "BH")

gg_hgTest_kappa_CEG <- ggplot(data = hgTest_kappa_CEG_DF,
                                 mapping = aes(x = group,
                                               y = log2obsexp,
                                               colour = pval,
                                               size = observed)) +
...
#  labs(size = "Count", colour = bquote("BH-adjusted" ~ italic(P))) +
  ...
  ylab(bquote(atop("Log"[2] * "(observed/expected) core eukaryotic genes"))) +
  ylim(-max(abs(hgTest_kappa_CEG_DF$log2obsexp)), max(abs(hgTest_kappa_CEG_DF$log2obsexp))) +
  theme_bw()
ggsave(paste0(plotDir_kappa_mC,
              ...
              "_CEG_genes_hypergeomTest_quantiles.pdf"),
       plot = gg_hgTest_kappa_CEG,
       height = 5, width = 6)
write.table(hgTest_kappa_CEG_DF,
            paste0(outDir,
                   sampleName, "_filt_df_fk_kappa_all_mean_mC_all_",
                   featName, "_", featRegion,
                   "_", paste0(chrName, collapse = "_"), "_", context,
...
# Run hypergeometric test on each group of genes to evaluate
# representation of CEG genes
hgTest_alpha_CEG_list <- lapply(seq_along(filt_alpha_mC_groups_featID), function(x) {
  hgTest(group = x,
...
hgTest_alpha_CEG_DF <- dplyr::bind_rows(hgTest_alpha_CEG_list)
hgTest_alpha_CEG_DF$group <- as.character(hgTest_alpha_CEG_DF$group)

hgTest_alpha_CEG_DF$BHadj_pval <- p.adjust(hgTest_alpha_CEG_DF$pval, method = "BH")

gg_hgTest_alpha_CEG <- ggplot(data = hgTest_alpha_CEG_DF,
                                 mapping = aes(x = group,
                                               y = log2obsexp,
                                               colour = pval,
                                               size = observed)) +
...
#  labs(size = "Count", colour = bquote("BH-adjusted" ~ italic(P))) +
  ...
  ylab(bquote(atop("Log"[2] * "(observed/expected) core eukaryotic genes"))) +
  ylim(-max(abs(hgTest_alpha_CEG_DF$log2obsexp)), max(abs(hgTest_alpha_CEG_DF$log2obsexp))) +
  theme_bw()
ggsave(paste0(plotDir_alpha_mC,
              ...
              "_", paste0(chrName, collapse = "_"), "_", context,
              "_CEG_genes_hypergeomTest_quantiles.pdf"),
       plot = gg_hgTest_alpha_CEG,
       height = 5, width = 6)
write.table(hgTest_alpha_CEG_DF,
            paste0(outDir,
                   sampleName, "_filt_df_ka_alpha_all_mean_mC_all_",
                   featName, "_", featRegion,
                   "_", paste0(chrName, collapse = "_"), "_", context,
...
# Run hypergeometric test on each group of genes to evaluate
# representation of CEG genes
hgTest_stocha_CEG_list <- lapply(seq_along(filt_stocha_mC_groups_featID), function(x) {
  hgTest(group = x,
...
hgTest_stocha_CEG_DF <- dplyr::bind_rows(hgTest_stocha_CEG_list)
hgTest_stocha_CEG_DF$group <- as.character(hgTest_stocha_CEG_DF$group)

hgTest_stocha_CEG_DF$BHadj_pval <- p.adjust(hgTest_stocha_CEG_DF$pval, method = "BH")

gg_hgTest_stocha_CEG <- ggplot(data = hgTest_stocha_CEG_DF,
                                mapping = aes(x = group,
                                              y = log2obsexp,
                                              colour = pval,
                                              size = observed)) +
...
#  labs(size = "Count", colour = bquote("BH-adjusted" ~ italic(P))) +
  ...
  ylab(bquote(atop("Log"[2] * "(observed/expected) core eukaryotic genes"))) +
  ylim(-max(abs(hgTest_stocha_CEG_DF$log2obsexp)), max(abs(hgTest_stocha_CEG_DF$log2obsexp))) +
  theme_bw()
ggsave(paste0(plotDir_stocha_mC,
              ...
              "_", paste0(chrName, collapse = "_"), "_", context,
              "_CEG_genes_hypergeomTest_quantiles.pdf"),
       plot = gg_hgTest_stocha_CEG,
       height = 5, width = 6)
write.table(hgTest_stocha_CEG_DF,
            paste0(outDir,
                   sampleName, "_filt_df_mean_stocha_all_mean_mC_all_",
                   featName, "_", featRegion,
                   "_", paste0(chrName, collapse = "_"), "_", context,
