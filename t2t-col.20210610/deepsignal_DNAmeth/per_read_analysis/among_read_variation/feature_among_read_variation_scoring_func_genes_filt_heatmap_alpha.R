...
plotDir_alpha <- paste0(outDir, "plots/heatmaps_", context, "_alpha/")
...
system(paste0("[ -d ", plotDir_alpha, " ] || mkdir -p ", plotDir_alpha))
...
featDF_alpha <- featDF_merge[
  with(featDF_merge, 
       order(alpha, decreasing = T)),
]
...
kappa_colFun <- colorRamp2(quantile(
    ...
    na.rm = T),
  heat.colors(6))
alpha_colFun <- colorRamp2(quantile(
    alpha_mat,
    c(0.05, 0.2, 0.4, 0.6, 0.8, 0.95),
    na.rm = T),
  heat.colors(6))
...
kappa_htmp <- featureHeatmap(mat = kappa_mat,
  colFun = kappa_colFun,
  datName = "Agreement kappa",
  rowOrder = c(1:nrow(kappa_mat)))
alpha_htmp <- featureHeatmap(mat = alpha_mat,
  colFun = alpha_colFun,
  datName = "Agreement alpha",
  rowOrder = c(1:nrow(alpha_mat)))
