...
# Analysis:
# Plot combined results of over- and under-representation analysis of genes grouped by both site-to-site stochasticity and mean methylation proportion

# Usage:
# conda activate R-4.0.0
# ./feature_among_read_variation_scoring_func_genes_pam_hypergeom_combined_bargraph_stocha.R Col_0_deepsignalDNAmeth_30kb_90pc t2t-col.20210610 CpG 0.50 'Chr1,Chr2,Chr3,Chr4,Chr5' 'gene' 'regions' 10000 1000
# conda deactivate
...
bp <- ggplot(data = combined,
  ...
  geom_segment(mapping = aes(x = 0.55, y = min(c(combined$log2obsexp, combined$log2alpha))-0.50,
               ...
               colour = brewer.pal(name = "Set1", n = 4)[1],
  ...
  geom_segment(mapping = aes(x = 1.55, y = min(c(combined$log2obsexp, combined$log2alpha))-0.50,
               ...
               colour = brewer.pal(name = "Set1", n = 4)[2],
               inherit.aes = F, size = 2) +
  geom_segment(mapping = aes(x = 2.55, y = min(c(combined$log2obsexp, combined$log2alpha))-0.50,
               ...
               colour = brewer.pal(name = "Set1", n = 4)[3],
               inherit.aes = F, size = 2) +
  geom_segment(mapping = aes(x = 3.55, y = min(c(combined$log2obsexp, combined$log2alpha))-0.50,
               ...
               colour = brewer.pal(name = "Set1", n = 4)[4],
               inherit.aes = F, size = 2) +

  xlab(bquote(atop("Stochasticity and mean m" * .(context), .(featName) ~ .(featRegion) ~ "cluster"))) +
