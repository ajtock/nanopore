...
# csmit -m 20G -c 1 "/applications/R/R-4.0.0/bin/Rscript feature_among_read_variation_scoring_func_genes.R Col_0_deepsignalDNAmeth_30kb_90pc t2t-col.20210610 CpG 0.50 1.00 'Chr1,Chr2,Chr3,Chr4,Chr5' 'gene'"
...
filt_kappa_mC_group_featID <- lapply(seq_along(filt_kappa_mC_groups), function(x) {
  sub(pattern = "\\.\\d+", replacement = "",
      x = filt_kappa_mC_groups[[x]]$name)
})


select(org.At.tair.db, keys = keys(org.At.tair.db), columns = c("TAIR"))
