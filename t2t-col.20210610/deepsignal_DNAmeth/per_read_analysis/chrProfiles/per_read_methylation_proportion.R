...
# csmit -m 100G -c 32 "/applications/R/R-3.5.0/bin/Rscript per_read_methylation_proportion.R"
 
# Calculate the per-read methylation proportion in windows along each read

# /applications/R/R-4.0.0/bin/Rscript per_read_methyl_density_heatmap.R Col_0_deepsignalDNAmeth_30kb_90pc t2t-col.20210610 1000 CpG
...
per_read_DNAmeth_DF <- do.call(rbind, mclapply(readIDs, function(x) {
  ...
  # Andy to Matt: Removed return(filt_methDat) as return() works only within a function (this may be the source of the issue you mentioned yesterday);
