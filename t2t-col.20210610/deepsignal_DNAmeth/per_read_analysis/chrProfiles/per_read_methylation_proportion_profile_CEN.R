...
# chmod +x per_read_methylation_proportion_profile_CEN.R
...
tab <- read.table(paste0("/home/ajt200/analysis/nanopore/", refbase, "/deepsignal_DNAmeth/",
                         sampleName, "_MappedOn_", refbase, "_", context, "_raw_grep_663b1a86-cf12-4aec-97bc-256bc8ceb57f.tsv"))
...
per_read_DNAmeth_DF <- do.call(rbind, mclapply(readIDs, function(x) {
  ...
  pdf(paste0("within_read_state_", context, "_raw_grep_663b1a86-cf12-4aec-97bc-256bc8ceb57f.pdf"),
      height = 5, width = 25)
  plot(x = y[,2], y = y[,9], type = "b", pch = 20, lwd = 0.25)
  dev.off()
