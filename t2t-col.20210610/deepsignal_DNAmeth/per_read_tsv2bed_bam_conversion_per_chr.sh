...
# csmit -m 400G -c 48 "bash ./per_read_tsv2bed_bam_conversion_per_chr.sh Col_0_deepsignalDNAmeth_30kb_90pc t2t-col.20210610 48 5G CHH" 
...
for chr in ${chrs};
do
  mtsv2bedGraph_deepsignal.py --input ${prefix}_MappedOn_${refbase}_${context}_raw_${chr}.tsv \
                              --mod ${context} \
                              --call-threshold 0.02 \
  ...
  cat ${prefix}_MappedOn_${refbase}_${context}_${chr}.bed
done | sort -k1,1 -k2,2n | bgzip > ${prefix}_MappedOn_${refbase}_${context}.bed.gz 
tabix -p bed ${prefix}_MappedOn_${refbase}_${context}.bed.gz
#${prefix}_MappedOn_${refbase}_${context}_${chr}.bed

# "Using the converted bed-style methylation file, the original bam file [e.g., generated by winnowmap]
# can be 'bisulfite converted in silico' for easy visualization on IGV via their bisulfite mode."
# See https://github.com/isaclee/nanopore-methylation-utilities
prefix_substr=$(echo ${prefix}_MappedOn_${refbase} | cut -d'_' -f 1,2,4-7)
...
convert_bam_for_methylation_deepsignal.py --threads ${threads} \
                                          ...
                                          --mod ${context_lower} | \
  /applications/samtools/samtools-1.9/samtools sort -@ ${threads} -m ${sortMem} -o ${prefix}_MappedOn_${refbase}_${context}_winnowmap_sorted_BSconverted_sorted.bam
/applications/samtools/samtools-1.9/samtools index ${prefix}_MappedOn_${refbase}_${context}_winnowmap_sorted_BSconverted_sorted.bam
