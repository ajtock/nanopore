...
# Convert centromeric TE-containing gaps and soloLTRs identified in t2t-col.20210610
...
# Usage:
# ./CENgap_CENsoloLTR_CSVtoBED_AlexBousios_strand_info_v040721.R 'Chr1,Chr2,Chr3,Chr4,Chr5'
...
tab <- read.csv("CENgap_in_t2t-col.20210610.csv",
                header = T)
colnames(tab)[1] <- "gap_name"
colnames(tab)[4] <- "gap_end"
colnames(tab)[12] <- "position"
colnames(tab)[13] <- "phylo"
tab$phylo <- toupper(tab$phylo)
colnames(tab) <- gsub(pattern = "\\.", replacement = "_", x = colnames(tab))
colnames(tab) <- gsub(pattern = "athila_", replacement = "", x = colnames(tab))
#tab$position <- paste0(tab$position, "_centr")

CENAthila <- tab[tab$position == "in",]
CENgap <- tab[tab$position == "in",]
CENsoloLTR <- tab[tab$start_diff == "SoloLTR",]
...
# Convert CENsoloLTR into GRanges and then BED format
CENsoloLTRGR <- GRanges(seqnames = CENsoloLTR$chr,
                        ranges = IRanges(start = CENsoloLTR$gap_start,
                                         end = CENsoloLTR$gap_end),
#                        strand = CENsoloLTR$strand,
                        strand = "+",
                        name = CENsoloLTR$gap_name,
