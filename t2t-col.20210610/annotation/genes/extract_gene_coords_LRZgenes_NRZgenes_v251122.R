...
# Region definitions
regions_DF <- read.csv("LRZ_NRZ/Table_S1_LRZ_NRZ.csv", header = T)[,-1]
...
LRZGR <- GRanges(seqnames = rep(regions_DF$Chr, 2),
                 ranges = IRanges(start = c(LRZstart, NRZend+1),
                                  end = c(NRZstart-1, LRZend)),
                 strand = "*")

NRZGR <- GRanges(seqnames = regions_DF$Chr,
                 ranges = IRanges(start = NRZstart,
                                  end = NRZend),
                 strand = "*")

armGR <- GRanges(seqnames = rep(regions_DF$Chr, 2),
