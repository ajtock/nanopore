...
awk 'BEGIN {OFS="\t"}; {print $1, ($2 - 1), $2}' 3a_SNPs_in_Col_and_Ws.tsv | \
  tail -n +2 > 3a_SNPs_in_Col_and_Ws.bed

# Create vcf output directory if it doesn't already exist
[ -d vcf ] || mkdir vcf
...
for x in $(seq 1 28800)
do
  echo "minimap2_TAIR10_${sample}_sorted_primary_MAPQ${MAPQ}_aln_${x}.bam" \
...
samtools mpileup --bam-list minimap2_TAIR10_${sample}_sorted_primary_MAPQ${MAPQ}_BAM_list.txt \
                 ...
                 --positions 3a_SNPs_in_Col_and_Ws.bed \
		 --ignore-RG \
                 --min-BQ 0 \
