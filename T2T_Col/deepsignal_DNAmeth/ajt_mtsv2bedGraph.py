...
import re
import pysam
import math # for calculating log ratios of "prob_1" (probability methylated) to "prob_0" (probability unmethylated)

def parseArgs():
    parser = argparse.ArgumentParser( description='Generate methylation bedGraph file')
    parser.add_argument('-v', '--verbose', action="store_true",required=False, default = False,
            help="verbose output")
    parser.add_argument('-c', '--call-threshold', type=float, required=False, default=0.02,
            help="absolute value of log(prob_1/prob_0) threshold for methylation call (default : 0.02)")
    parser.add_argument('-i', '--input', type=str, required=False,help="input methylation tsv file (default stdin)")
    parser.add_argument('-q', '--mod',type=str,required=False,default='CpG',help="modification motif; one of CpG,CHG,CHH,GpC,DAM,CpGGpC")
    ...
    parser.add_argument('--offset',type=int,required=False,default=0,
            help="DeepSignal coordinate offset (0-based)")
...
class readQuery:
    ...
    def update(self,record,seq_dict):
        llr=math.log((float(record['prob_1'])+0.0001) / (float(record['prob_0'])+0.0001))
...
#        sequence = record['sequence']
        if record['strand'] == '+' :
            ...
            start = int(record['start'])-self.win_size-(len(self.motif)-1)
#        sequence = seq_dict[self.rname][start:int(record['end'])+2+self.win_size].upper()
        sequence = seq_dict[self.rname][start:int(record['start'])+2+self.win_size].upper()
...
#            pos = sequence.find(self.motif,pos+1)
        seq = sequence[pos-self.win_size:pos+self.win_size+(len(self.motif)-1)]
        if not self.nome :
            pos_list.append(pos)
            self.ratio.append(llr)
            self.call.append(call)
