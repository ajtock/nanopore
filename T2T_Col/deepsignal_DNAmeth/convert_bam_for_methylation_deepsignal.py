...
def parseArgs() :
    ...
    parser.add_argument('-v','--verbose', action='store_true',default=False,
            help="verbose output")
    parser.add_argument('-c','--mbed',type=os.path.abspath,required=True,
            help="methylation bed - sorted, bgzipped, and indexed")
    parser.add_argument('-q','--mod',type=str,required=True,default="cpg",
            help="modification motif; one of cpg,chg,chh,gpc")
...
def change_sequence(bam,calls,mod="chg") :
    ...
    if mod == "cpg" :
        ...
        seq = np.array(list(bam.query_sequence.replace("CHG",trinuc)))
    elif mod == "chh" :
        seq = np.array(list(bam.query_sequence.replace("CHH",trinuc)))
#    seq[gsites-offset] = g
...
def main() :
    ...
    if args.gpc is not None : converter = convert_nome
    elif args.mod == "cpg" : converter = convert_cpg
    elif args.mod == "chg" : converter = convert_chg
    elif args.mod == "chh" : converter = convert_chh
    # start processing
    if args.fasta : 
        jobs = list()
        for win in windows : 
            ...
            jobs.append(pool.apply_async(convertBam,
                args = (args.bam,seq,converter,args.mbed,args.gpc,
                    win,args.remove_poor,args.verbose,q)))
    else : 
        jobs = [ pool.apply_async(convertBam,
            args = (args.bam,0,converter,args.mbed,args.gpc,
                win,args.remove_poor,args.verbose,q))
            for win in windows ]
