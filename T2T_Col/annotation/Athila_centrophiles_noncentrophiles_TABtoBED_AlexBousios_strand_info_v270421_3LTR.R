...
# Usage:
# ./Athila_centrophiles_noncentrophiles_TABtoBED_AlexBousios_strand_info_v270421_3LTR.R 'Chr1,Chr2,Chr3,Chr4,Chr5'
...
# Use genome_3LTR_start and genome_3LTR_end as
...
CENAthilaGR <- sort(c(
                      GRanges(seqnames = CENAthila_forward$chr,
                              ranges = IRanges(start = CENAthila_forward$genome_3LTR_start,
                                               end = CENAthila_forward$genome_3LTR_end),
                      ...
                      GRanges(seqnames = CENAthila_reverse$chr,
                              ranges = IRanges(start = CENAthila_reverse$genome_3LTR_end,
                                               end = CENAthila_reverse$genome_3LTR_start),
...
write.table(CENAthila_bed,
            file = paste0(CENAthilaDir, "CENAthila3LTR_in_T2T_Col_",
...
nonCENAthilaGR <- sort(c(
                         GRanges(seqnames = nonCENAthila_forward$chr,
                                 ranges = IRanges(start = nonCENAthila_forward$genome_3LTR_start,
                                                  end = nonCENAthila_forward$genome_3LTR_end),
                         ...
                         GRanges(seqnames = nonCENAthila_reverse$chr,
                                 ranges = IRanges(start = nonCENAthila_reverse$genome_3LTR_end,
                                                  end = nonCENAthila_reverse$genome_3LTR_start),
...
write.table(nonCENAthila_bed,
            file = paste0(nonCENAthilaDir, "nonCENAthila3LTR_in_T2T_Col_",
                          paste0(chrName, collapse = "_"), ".bed"),
            quote = F, sep = "\t", row.names = F, col.names = F)
